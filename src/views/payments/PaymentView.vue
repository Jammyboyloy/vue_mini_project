            ...
            <div class="col-lg-7">
                ...
                <div>
                    ...
                    <Transition name="swap" mode="out-in">
                        <!-- Has file -->
                        <div v-if="fileName" key="file-row"
                            ...
                            <div class="d-flex align-items-center justify-content-center rounded-2 flex-shrink-0">
                                <i :class="[fileIcon, 'text-main']"
                                    :style="{ fontSize: '1.25rem', color: justUploaded ? 'var(--pickup)' : 'var(--accent)' }"></i>
                            </div>
                    ...
                    <!-- Upload zone -->
                    <label class="d-flex align-items-center justify-content-center rounded-3 p-3 w-100 upload-zone "
                        ...
                        <div class="d-flex align-items-center gap-3">
                            <i class="bi bi-cloud-arrow-up upload-icon"
                                style="font-size:1.5rem;color:var(--muted);transition:color .2s;"></i>
                            <div class="d-flex flex-column gap-1">
                                <span class="fw-bold" style="font-size:.85rem;color:var(--ink);">
                                    {{ fileName ? 'Replacefile' : 'Attach file' }}</span>
                                <span class="text-muted" style="font-size:.68rem;">drag & drop or click to browse</span>
                            </div>
                        </div>
                ...
                <div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-6">
                            <button
                                ...
                                :class="{ 'bg-btn text-white': modelValue === 1 }" @click="switchMethod(1)">
                                <div
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <span class="d-block text-muted">Customer collects order</span>
                                </div>
                        ...
                        <div class="col-6">
                            <button
                                class="method-card w-100 d-flex align-items-center gap-3 p-3 rounded-3 text-start shadow-sm"
                                :class="{ 'bg-btn text-white': modelValue === 2 }" @click="switchMethod(2)">
                                <div
                                    class="method-icon d-flex align-items-center justify-content-center rounded-2 flex-shrink-0">
                    ...
                    <Transition name="slide-fade">
                        <div v-if="modelValue">
                            ...
                            <!-- Location card -->
                            <div class="rounded-3 p-3 location-card shadow-sm"
                                :class="{ 'card-filled': localPlace && !isEditing }">
                                <template v-if="localPlace && !isEditing">
                                    <div class="d-flex align-items-center gap-3">
                                        <i class="bi bi-map-fill flex-shrink-0" style="font-size:1.1rem;"></i>
                                        <div class="flex-grow-1 overflow-hidden">
                                            <span class="d-block fw-medium text-truncate"
                                                style="font-size:.82rem;color:var(--ink);">{{ localPlace }}</span>
                                            <span class="d-block text-main" style="font-size:.62rem;">Tap Edit to
                                                change · Opens in Google Maps</span>
                                        </div>
                                        <a :href="computedMapUrl" target="_blank" rel="noopener"
                                            class="d-flex align-items-center justify-content-center rounded-2 flex-shrink-0 text-decoration-none icon-btn"
                                            @click.stop style="color:var(--accent2);">
                                ...
                                <template v-else>
                                    ...
                                    <div class="position-relative">
                                        <i class="bi bi-search position-absolute top-50 translate-middle-y text-muted"
                                            style="left:13px;font-size:.82rem;pointer-events:none;"></i>
                                        <input class="form-control search-input themed-input pe-5"
                                            :class="{ 'input-filled': localPlace }" type="text" v-model="localPlace"
                                            @input="onInput" @keydown.enter="confirmPlace"
                                            :placeholder="modelValue === 1 ? 'Branch name or address\u2026' : 'Delivery address or place name\u2026'" />
                                    ...
                                    <div class="d-flex align-items-start gap-2 mt-2">
                                        ...
                                        <span class="text-muted">
                                            Type a place name or address — the map preview on the right will update
                                            automatically
                                        </span>
                                    </div>
                                </template>
            ...
            <div class="col-lg-5">
                ...
                <div>
                    ...
                    <div class="rounded-3 overflow-hidden border-main">
                        ...
                        <!-- Dark header -->
                        <div class="d-flex align-items-end justify-content-between px-4 py-3 bg-btn">
                            <div class="d-flex flex-column gap-1">
                                <span class="text-uppercase fw-semibold text-white">Amount Due</span>
                                <span class="fw-black text-white" style="font-size:1.8rem;line-height:1;">
                        ...
                        <div class="d-flex justify-content-center align-items-center p-4">
                            <div class="d-flex justify-content-center align-items-center w-75">
                                <img src="https://qrcg-free-editor.qr-code-generator.com/latest/assets/images/websiteQRCode_noFrame.png"
                                    class="img-thumbnail" alt="">
                            </div>
                        </div>

                        <div class="text-center">
                            <span class="text-uppercase fw-semibold text-main"
                ...
                <div class="mt-5">
                    ...
                    <div class="rounded-3 overflow-hidden map-card">
                        <div class="position-relative map-frame">
                            <Transition name="map-fade" mode="out-in">
                                <!-- Live map iframe -->
                                <iframe v-if="debouncedPlace" :key="debouncedPlace" :src="embedUrl" allowfullscreen
                                ...
                                <!-- Typing state -->
                                <div v-else-if="isTyping" key="typing"
                                    ...
                                    <div class="text-center">
                                        <p class="fw-bold mb-1" style="font-size:.88rem;">Looking up
                                            location…</p>
                                        <p class="text-muted mb-0" style="font-size:.68rem;">Map will appear when you
                                            stop typing
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div v-else key="empty"
                                    ...
                                    <i class="bi bi-map"></i>
                                    <div class="text-center">
                                        <p class="fw-bold mb-1">No location
                                            yet</p>
                                        <p class="text-muted mb-0 lh-base" style="font-size:.68rem;">
                                            Type a place name in the field on the left<br />and the map will update here
                                            automatically
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div class="d-flex align-items-center gap-2 px-3 py-2 map-footer">
                                <i class="bi bi-geo-alt flex-shrink-0" style="font-size:.9rem;color:var(--accent);"></i>
                                <div class="flex-grow-1 overflow-hidden">
                                    <span class="d-block text-truncate" style="font-size:.7rem;color:var(--ink);">
                                        {{ debouncedPlace || placeName || 'No location set' }}
                                    </span>
                                    <span v-if="!debouncedPlace && !placeName" class="d-block text-muted"
                                        style="font-size:.62rem;">
                                        Enter a location above to enable map
                                    </span>
                                </div>
                                <a v-if="computedMap" :href="computedMap" target="_blank" rel="noopener"
                    ...
                    <div class="d-flex align-items-start gap-2 mt-2 ms-4">
                        <i class="bi bi-info-circle flex-shrink-0 mt-1" style="font-size:.72rem;"></i>
                        <span class="text-muted" style="font-size:.63rem;line-height:1.5;">
                            Map updates automatically 0.8 s after you stop typing a location name
                        </span>
                        <div class="d-flex justify-content-end mt-4">
                            <button class="btn bg-btn d-flex align-items-center gap-2 fw-bold px-4 py-3"
                                @click="$emit('save')">
...
const emit = defineEmits(['file-changed', 'update:modelValue', 'update:address', 'update:googleMapUrl', 'update:amount', 'save'])
...
const previewFile = () => { if (fileUrl.value) window.open(fileUrl.value, '_blank') }
const downloadFile = () => {
    if (!fileUrl.value) return
    const a = document.createElement('a')
    a.href = fileUrl.value; a.download = props.fileName; a.click()
}
...
const parseFile = (file) => {
    ...
    emit('file-changed', { name: file.name, size: `${(file.size / 1024).toFixed(0)} KB · ${ext}`, file })
...
const formatCurrency = (val) =>
    new Intl.NumberFormat('kh-KH', { style: 'currency', currency: 'KHR' }).format(val || 0)
...
watch(placeName, (val) => {
    clearTimeout(timer)
    if (!val.trim()) { isTyping.value = false; debouncedPlace.value = ''; return }
    isTyping.value = true
    timer = setTimeout(() => { debouncedPlace.value = val.trim(); isTyping.value = false }, 800)
}, { immediate: true })
